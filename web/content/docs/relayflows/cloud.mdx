...
An authored `.flow.ts` takes `--input` exactly as a local run does (an existing JSON file, otherwise inline JSON) and travels as one self-contained source: `use:` dependencies and sibling imports are refused before any HTTP call, since the hosted runner loads the flow from the request, not from a checkout. **Verified broken today**: the third command above — `flows run --cloud <flow.ts> --input <json>`, with or without `--wait` — fails immediately with `{"ok":false,"code":"http_error","message":"Cloud request failed with HTTP 400."}` against a real, `flows check`-passing authored flow. Only the declarative YAML/JSON form (the first two commands) actually submits a run. Use `flows deploy` for a TS flow today; the one-shot cloud path for authored TypeScript is tracked as broken (flows#461).
...
**Verified broken today, same as above**: this exact command, run against a real git checkout with a `flows check`-passing `review.flow.ts`, fails with the same `http_error: HTTP 400` — `--sync-code` doesn't change the outcome, since the failure is in submitting a TS-authored flow to `run --cloud` at all, before code sync would even matter. `flows sync <run-id>` itself (and the `git apply`/`patch_conflict` mechanics below) could not be verified in this pass, since no TS `--cloud` run reached completion to sync from.
