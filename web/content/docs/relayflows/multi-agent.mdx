...
`Ctx` declares both verbs, and this typechecks and passes `flows check` — but neither runs yet in 2.0.16. Verified directly: `flows run` on a flow that reaches `f.human` fails with `unsupported_verb: the initial authored executor does not lower f.human`, and the same for `f.dispatch`. Treat the shape below as the intended design, not something to ship on today:
