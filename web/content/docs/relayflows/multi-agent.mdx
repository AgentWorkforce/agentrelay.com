...
Each step's `agent:` selector resolves to that named `{ cli, model }` pair at compile time — the kernel never guesses a default. `flows check` also flags a named agent nobody ever selects, and one that a step overrides without using, so a stale declaration doesn't quietly rot in the spec.
...
<Note>
  ...
  takes `{ task, workspace }` — `name` just labels the call in the journal.
  It doesn't select a `cli`/`model` pair the way a YAML `agent:` selector does.
</Note>
