...
<Note>
  The named-agent **map** (`agents:` plus a step's `agent:` selector,
  reused across steps by name) is YAML/JSON authoring only today. In
  TypeScript, `f.agent(name, options)` takes `{ task, workspace?, cli?,
  model? }` ([flows#310](https://github.com/AgentWorkforce/flows/issues/310))
  — so a call can set its own `cli`/`model` directly, but the `name`
  argument still just labels the call in the journal; there's no map to
  select a declared pair from, or to reuse across multiple calls, the
  way YAML's `agent:` selector does ([flows#300](https://github.com/AgentWorkforce/flows/issues/300)).
</Note>
