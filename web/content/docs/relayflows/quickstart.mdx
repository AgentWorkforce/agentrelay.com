...
`flows run` starts a background daemon, `relayflowd`, and that's where the journal actually lives — not in the CLI process you typed the command into. Close the terminal, lose power, or kill the daemon itself mid-run, and nothing is lost:
