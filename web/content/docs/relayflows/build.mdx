...
An `agent` step declares what it's allowed to touch — `fileGlobs`, `accessPreset` — and what happens if it crashes mid-edit. These are YAML step fields; the TypeScript body reaches the same step through `f.dispatch`, or leaves recovery to the kernel's default (`reset`):
