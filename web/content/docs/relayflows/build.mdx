...
An `agent` step declares what it's allowed to touch — `fileGlobs`, `accessPreset` — and what happens if it crashes mid-edit. These are YAML step fields with no TypeScript equivalent; `f.dispatch` is not a working way to reach them today (see above — it doesn't execute in 2.0.16). Author the step in YAML if you need `permissions`/`recoveryMode`, or leave recovery to the kernel's default (`reset`) in TypeScript:
