...
The YAML `edit` step above names its own `cli` and `model`. TypeScript has no such fields to set on an `f.agent` call yet ([tracked upstream](https://github.com/AgentWorkforce/flows/issues/310)) — it needs a `flows.json` with a project-level `cli` next to `hello-agent.flow.ts`, or `flows run` refuses it before anything runs. See [Introduction](/docs/relayflows/introduction#a-whole-flow) for exactly how that resolution works.
