...
The YAML step above names its own `cli` and `model` directly. TypeScript's `f.agent` has neither field — `Ctx.agent`'s options are just `{ task, workspace? }`. Its CLI instead falls back to the nearest `flows.json`'s project-level default:

```json
{ "cli": "claude" }
```

Without one, the step has no CLI at step, flow, or project level, and `flows run` refuses before anything is journaled — exit `2`, `REFUSED [invalid_spec]`. There's no equivalent fallback for a model: an authored TypeScript step has no field to declare one, so it just runs whatever model its resolved CLI defaults to.
