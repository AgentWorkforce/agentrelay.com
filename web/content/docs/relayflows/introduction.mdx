...
The YAML step above names its own `cli` and `model` directly. TypeScript can't do that yet — `Ctx.agent`'s options are just `{ task, workspace? }`, with no `cli` or `model` at all ([tracked upstream](https://github.com/AgentWorkforce/flows/issues/310)). Today, an authored `f.agent` call gets its CLI from a project-wide `flows.json` default instead, and has no way to pin a model:

```json
{ "cli": "claude" }
```

Without a `flows.json` (or one with no `cli`), the step has no CLI at step, flow, or project level, and `flows run` refuses before anything is journaled — exit `2`, `REFUSED [invalid_spec]`.
