...
The YAML step above names its own `cli` and `model` directly. TypeScript's `f.agent` has no `cli`/`model` fields at all — it resolves both from the nearest `flows.json`:
...
Without one, an authored agent step refuses to run as `agent_cli_unresolved` rather than guessing a CLI.
