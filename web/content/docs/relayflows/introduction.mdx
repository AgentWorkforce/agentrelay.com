---
title: 'Relayflows'
description: 'A deterministic script over agentic primitives: steps you can inspect, verify, and resume.'
---

Relayflows turns a coding-agent task into steps you can inspect and verify. A flow combines shell commands, model calls, and coding agents with a journal that records what each step did and why it completed. Start with a small local flow, then add verification as the task grows.
...
// flows.json — next to hello.flow.ts. f.agent has no cli/model options of
// its own, so this is where its CLI comes from.
// { "cli": "claude" }
...
The YAML step above names its own `cli` and `model` directly. TypeScript's `f.agent` has neither field, so its CLI comes from the `flows.json` shown above instead — `Ctx.agent`'s options are just `{ task, workspace? }`. Without a `flows.json` (or one with no `cli`), the step has no CLI at step, flow, or project level, and `flows run` refuses before anything is journaled — exit `2`, `REFUSED [invalid_spec]`. There's no equivalent fallback for a model: an authored TypeScript step has no field to declare one, so it just runs whatever model its resolved CLI defaults to.
