...
flows run --cloud --wait my-flow.flow.ts --input '{}'    # hosted authored flow
```

An authored `.flow.ts` requires `--input`: an existing JSON file, otherwise inline JSON (up to 1 MiB), handed to the body as its second argument.
