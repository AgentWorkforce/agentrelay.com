...
Validates the spec — schema, step graph, verification blocks, and whether every declared CLI actually exists and is authenticated. Nothing runs; nothing is spawned. This is the same preflight a run does before its first step, exposed on its own so a broken spec fails in CI, not at minute 27 of a real run.
...
`--local-agent` attaches the existing SDK agent worker to your local daemon and runs the declared CLI (`claude`, `codex`, …) with its own local access. It's what makes an authored TypeScript flow's `agent` steps runnable without a separate credential path.
...
`tick` is the general-purpose durable schedule: a deterministic-id claim on each interval, with catch-up bounded by `--max-catch-up` if the process was down when an interval fired. `hn-monitor` is a named, narrower proactive trigger built the same way. Both are the same idea underneath: a schedule that reports whether it's still alive, not one that silently stops firing for weeks and reports nothing.
