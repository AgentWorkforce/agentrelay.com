...
Relay does not care whether a harness delivers through a PTY, a headless SDK callback, an app-server API, a webhook, a queue, or a native MCP notification. It cares about the semantic delivery mode, the message, the session capabilities, and the receipt.

Delivery is routed through [nodes](/docs/nodes). Direct SDK and MCP clients use implicit `direct_ws` nodes, broker-controlled workers use `fleet_ws` nodes, and service endpoints can use `http_push` nodes. The node binding for an agent decides which adapter receives future delivery work.
...
Every session must be able to receive a message and be released.
...
`immediate` is the interrupting mode. The other modes are boundary-aware.
...
If a WebSocket is not available, delivery can still work through polling, queue workers, app-server webhooks,
HTTP push nodes, or MCP-triggered flush tools. The stored message and delivery record remain the source of
truth.
