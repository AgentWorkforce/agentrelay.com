...
```bash
relayfile listen default \
  --path "/github/repos/acme/api/pulls/**" \
  --event file.created \
  --run "review-bot start {{path}}"
```

Its full surface is `relayfile listen [WORKSPACE] [--provider PROVIDER] [--path GLOB] [--event TYPE] [--run CMD] [--format text|json] [--background]`. A delivered event carries `eventId`, `type`, `path`, `revision`, `provider`, `origin`, `correlationId`, and — for small files — the content inlined, so the handler usually needs no follow-up read:
