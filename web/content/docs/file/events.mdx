...
Filter by path glob and event type so an agent only wakes for what it cares about. To fan events out to a channel, the CLI binds a path glob to a webhook:

```bash
relayfile integration bind linear "/linear/issues/by-state/triage/**" \
  --channel triage --webhook "$WEBHOOK_ID" --webhook-token "$WEBHOOK_TOKEN"
```

To react in your own process, use the SDK's `onWrite`, which subscribes over the same WebSocket stream and dispatches by pattern:
