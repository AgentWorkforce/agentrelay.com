...
Rust provider that spawns and owns agent processes. A caller uses it to spawn a PTY and drive it. When a
`spawn:<harness>` invoke lands on a node, its agent runtime resolves the harness and drives the PTY through
this package. It is not a standalone service.
...
`AgentWorkforce/agent-relay-acp-bridge` repository.
