...
<Note>
  ...
  They are distinct from a [node](/docs/nodes)'s **capabilities**, which are the named `spawn` and `action`
  operations a node advertises to the fleet for [placement](/docs/nodes#placement).
</Note>
...
Capability errors are explicit and structured — never silent message loss.
