...
<Note>
  Local `ai-hist stats` reports entry counts, sources, date range, and top projects. Cost and token attribution — the Burn lens below — is built on the per-event token and cost fields (`inputTokens`, `outputTokens`, `costUsdMicros`, `model`, `provider`) captured in [Relayloop Cloud](/docs/loop/cloud), not in the local overview.
</Note>
