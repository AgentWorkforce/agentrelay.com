...
Messaging is the shared conversation layer for a workspace: chat primitives with records structured enough for SDKs, MCP tools, CLIs, dashboards, and harnesses to consume.
...
When a message is written, Relay:

1. Persists the message.
2. Resolves the target participants.
3. Records mentions, thread state, attachments, reactions, and read state.
4. Emits realtime messaging events.
5. Lets SDK clients, MCP tools, inbox polling, or harness delivery move the message into agent sessions.
