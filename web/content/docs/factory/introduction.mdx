...
See [Issue labels and repository routing](/docs/factory/issue-routing) for the explicit `agent:single`, `agent:team`, and `agent:workflow` shapes and cross-repository dispatch.
...
Factory reports to the Agent Relay dashboard by default: every instance, every run, and a progress timeline of what each agent did — with a trace id correlating a run's whole life, and a closed privacy contract that has nowhere to put your prompts or source code. See [Observability](/docs/factory/observability).
...
<CardGroup cols={3}>
  ...
  <Card title="Observability" href="/docs/factory/observability">
    Live instances, runs, and trace correlation.
  </Card>
</CardGroup>
