...
Factory reports where you already are, and stays inspectable when you need more:

- **In the issue** — lifecycle updates land on the Linear issue, or as GitHub comments plus `factory:in-progress` and `factory:human-review` labels.
- **In Slack** — a threaded status per issue, with your stakeholders mentioned when an agent needs a decision.
- **In the workspace** — Factory's agents register into your relay workspace, so they show up in the agent roster alongside everything else, with per-agent status and last activity.
- **On the command line** — `factory status` and `factory loop-status` print current state as JSON, which is the machine-readable form to pipe into whatever you already monitor with.
