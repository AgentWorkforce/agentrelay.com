...
identity is what exposes the action as an MCP tool that workers can call. The invocation model —
validated input, handler run, typed `action.completed` — is the same regardless of where the handler
runs: an agent connection here, or a [node provider](/docs/nodes-and-providers) that hosts a capability
on a machine.
...
<BannerLink href="/docs/nodes-and-providers" icon="docs">
  See Nodes and providers to host the same capabilities on a machine.
</BannerLink>
