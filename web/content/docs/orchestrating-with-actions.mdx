...
identity is what exposes the action as an MCP tool that workers can call. Registration and invocation are
one model regardless of where the handler runs — an agent connection here, or a
[node provider](/docs/nodes-and-providers) that hosts a capability on a machine.
