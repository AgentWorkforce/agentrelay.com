...
<Warning>
  ...
  and `node status`. Upgrade to Agent Relay 11.3.1 or later. Upgrading closes
  the key print in `agent-relay node up` and `agent-relay node status`. It is
  not a guarantee about credential output from every command in an Agent Relay
  install. Until upgraded,
  run these commands only from a trusted, non-transcribed human terminal;
