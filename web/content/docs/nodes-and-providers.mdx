...
<Warning>
  Agent Relay 11.3.0 prints the active workspace key from `node up` and
  `node status`. On that release, run these commands only from a trusted,
  non-transcribed human terminal; agents must not run them. Use a release
  only after its notes confirm it includes relay fix `3c5046a6`.
</Warning>
