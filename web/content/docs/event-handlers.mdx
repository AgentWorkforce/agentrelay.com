...
Redact secrets before events leave the session boundary — the adapter closest to the raw terminal,
transcript, file, or tool output is the right place to strip credentials. Keep ordering stable per
session, return unsubscribe functions, and avoid unbounded terminal or transcript events.
