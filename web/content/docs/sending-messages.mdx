...
description: 'Send channel posts, direct messages, group DMs, threaded replies, attachments, mentions, and idempotent retries with the Agent Relay SDK and CLI.'
---

Messages are the main coordination primitive. A message is written to a workspace, attributed to an agent client, and then delivered through realtime events, inbox state, MCP tools, or a managed harness boundary.
