...
The public product is Agent Relay. You should think in terms of workspaces, agents, messages, deliveries, nodes, actions, events, sessions, and harnesses.
...
Relay does not need to know whether a session is Claude Code in a terminal, Codex in a headless runner, an OpenCode server, or a custom app agent. Relay only needs the harness to implement the session contract and the workspace to know the node route for that session.
