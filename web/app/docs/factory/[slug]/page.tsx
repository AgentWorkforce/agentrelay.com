...
const SECTION_ID = factorySection.id;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${doc.frontmatter.title} — Factory`,
    ...
    openGraph: {
      ...
      type: 'article',
      images: [ogImage(ogPath, `${doc.frontmatter.title} — Factory docs`)],
    },
    twitter: {
...
export default async function FactoryDocPage({ params }: PageProps) {
  const { slug } = await params;
